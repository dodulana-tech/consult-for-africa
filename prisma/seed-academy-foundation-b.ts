/**
 * CFA TRAINING ACADEMY - FOUNDATION LEVEL SEED B (Tracks 3-5)
 * Seeds Foundation-level training tracks, modules, and assessment questions
 *
 * 3 Tracks:
 *   Track 3: CFA Platform & Engagement
...
async function main() {
  console.log('Seeding CFA Training Academy - Foundation Level Tracks (3-5)...\n')

  // ════════════════════════════════════════════════════════════════════════════
  // TRACK 3: FOUNDATION - CFA Platform & Engagement
  // ════════════════════════════════════════════════════════════════════════════

  const platformEngagement = await prisma.trainingTrack.create({
    data: {
      name: 'CFA Platform & Engagement',
      slug: 'cfa-platform-engagement',
      description: `Understand how CFA engagements are structured from first client contact to final handover,
        and learn to navigate the CFA digital platform for day-to-day project execution. This track covers
        the engagement lifecycle, deliverable standards, quality gates, and practical use of the CFA platform
  ...
  // Module 3.1: CFA Engagement Lifecycle
  const m3_1 = await prisma.trainingModule.create({
    data: {
      trackId: platformEngagement.id,
      name: 'CFA Engagement Lifecycle',
      slug: 'cfa-engagement-lifecycle',
      description: 'Master the four phases of a CFA engagement, understand the deliverables expected at each stage, and learn how quality gates ensure consistent output across every project.',
      order: 1,
      estimatedMinutes: 90,
      passingScore: 80,
      content: {
    ...
    data: [
      {
        ...
        question: 'What are the four phases of the CFA engagement lifecycle in order?',
        ...
        explanation: 'CFA uses the Diagnose-Design-Deliver-Sustain (DDDS) framework as its standard operating model for all engagements, ensuring consistency across projects of all sizes.',
        points: 1,
        order: 1,
      },
      {
      ...
      },
      {
        moduleId: m3_1.id,
        type: 'MULTIPLE_CHOICE',
        question: 'What is the minimum review process for CFA deliverables before they reach the client?',
        options: JSON.stringify([
        ...
        explanation: 'CFA requires at least two sets of eyes on every deliverable. Junior consultants draft, senior consultants review, and the engagement lead approves. This is non-negotiable.',
        points: 1,
        order: 4,
      },
      {
        moduleId: m3_1.id,
        type: 'MULTIPLE_CHOICE',
        question: 'The Sustain phase ensures changes stick after CFA leaves. Which of these is a Sustain-phase deliverable?',
        options: JSON.stringify([
        ...
        explanation: 'The Sustain phase produces a Handover Pack, a Sustainability Scorecard, and a 90-day Follow-Up Plan. These tools ensure the client can maintain improvements independently after CFA exits.',
        points: 1,
        order: 5,
      },
    ],
  ...
  // Module 3.2: Using the CFA Platform
  const m3_2 = await prisma.trainingModule.create({
    data: {
      trackId: platformEngagement.id,
      name: 'Using the CFA Platform',
      slug: 'using-cfa-platform',
      description: 'Learn to navigate the CFA digital platform for project management, timesheet submission, document collaboration, and accessing the firm knowledge base.',
      order: 2,
      estimatedMinutes: 90,
      passingScore: 80,
      content: {
        sections: [
          {
            ...
            type: 'text',
            body: `The CFA platform is the single source of truth for every active engagement. When you are assigned to a project, it appears on your dashboard with a summary of the current phase, your assigned tasks, upcoming deadlines, and the engagement lead's contact details. Every task in the platform maps to a deliverable or workstream from the Transformation Roadmap.
    ...
    data: [
      {
        moduleId: m3_2.id,
        type: 'MULTIPLE_CHOICE',
        question: 'When must weekly timesheets be submitted on the CFA platform?',
        options: JSON.stringify([
        ...
        explanation: 'CFA has a firm-wide policy requiring timesheets to be submitted by end of day Friday each week, with no exceptions.',
        points: 1,
        order: 1,
      },
      {
        moduleId: m3_2.id,
        type: 'MULTIPLE_CHOICE',
        question: 'What is the target utilization rate for CFA consultants?',
        options: JSON.stringify([
        ...
        explanation: 'CFA targets 70-80% utilization. Below 70% means the consultant is under-deployed. Above 85% consistently signals burnout risk.',
        points: 1,
        order: 2,
      },
      {
        moduleId: m3_2.id,
        type: 'MULTIPLE_CHOICE',
        question: 'What is the correct CFA document naming convention?',
        options: JSON.stringify([
        ...
        explanation: 'CFA uses the naming convention [Project Code]-[Phase]-[Document Type]-[Version], for example NGA-LAG-001-DIAGNOSE-CSA-v2.',
        points: 1,
        order: 3,
      },
      {
        ...
        options: JSON.stringify([
          ...
          { id: 'c', text: 'Move the task to In Review status on the CFA platform', isCorrect: true },
          { id: 'd', text: 'Upload to a shared Google Drive folder', isCorrect: false },
        ]),
        explanation: 'All deliverables must go through the CFA platform by moving tasks to In Review status. This ensures a clear audit trail. Email and WhatsApp should not be used for deliverable submissions.',
        points: 1,
        order: 4,
      },
      {
        moduleId: m3_2.id,
        type: 'MULTIPLE_CHOICE',
        question: 'Before starting a new analysis, what should a CFA consultant do first?',
        options: JSON.stringify([
          { id: 'a', text: 'Build the analysis from scratch to ensure originality', isCorrect: false },
          { id: 'b', text: 'Check the CFA Knowledge Base for similar past work that can be adapted', isCorrect: true },
        ...
        explanation: 'The CFA Knowledge Base contains anonymized past deliverables, methodology guides, and templates. Checking it first avoids reinventing the wheel and ensures consistency with proven CFA approaches.',
        points: 1,
        order: 5,
      },
    ],
  ...
  console.log('  Track 3: CFA Platform & Engagement - 2 modules, 10 questions')
        ...
        sections: [
          {
            ...
            body: `Hospital revenue in Africa typically comes from four sources: out-of-pocket payments, health insurance (NHIS, private HMOs), government subsidies or grants, and development partner funding. The mix varies dramatically by institution type. A private hospital in Lagos might derive 60% of revenue from HMO claims and 35% from out-of-pocket, while a mission hospital in rural Tanzania might depend on 40% government grants, 30% development partner funding, and 30% patient fees on a sliding scale.
              ...
              Understanding revenue mix is critical because each source has different collection dynamics. Out-of-pocket is collected at point of service but limits patient volume. Insurance revenue is higher volume but comes with 60-120 day payment cycles and claim rejection rates of 15-30% in some markets. Government funding is predictable in theory but often arrives late. A CFA consultant must map these dynamics early in any financial diagnostic.`
          },
          {
            ...
            body: `Hospital costs fall into three categories: personnel (typically 45-60% of total costs), supplies and consumables (20-30%), and overhead including utilities, maintenance, and administration (15-25%). Personnel costs are largely fixed in the short term since you cannot lay off nurses when occupancy drops. This makes hospitals operationally leveraged, meaning that small changes in revenue have outsized impacts on profitability.
              ...
              Gross margin in African hospitals ranges from 25-45% for private facilities and is often negative for public and mission hospitals that rely on subsidies. Operating margin after overhead for well-run private hospitals sits between 8-15%. Below 5% operating margin, a hospital is one bad quarter away from cash flow crisis. CFA consultants should benchmark client margins against these ranges to quickly identify whether the problem is revenue, cost, or both.`
          },
          {
        ...
        sections: [
          ...
          },
          {
            ...
            body: `Variance analysis compares budgeted figures to actual results and investigates the reasons for differences. A favorable variance means actual performance exceeded budget (higher revenue or lower costs). An unfavorable variance means the opposite. But the label alone is not enough. You must understand whether the variance is due to volume (more or fewer patients), price/rate (different billing rates or procurement costs), or efficiency (more or fewer resources consumed per unit of output).
              ...
              For example, if pharmacy costs are 20% over budget, the variance could be a volume variance (more patients required more drugs), a price variance (supplier prices increased), or an efficiency variance (clinical staff are prescribing more expensive drugs than the formulary recommends). Each root cause demands a different response. CFA consultants must decompose variances rather than simply reporting that a line item is over or under budget.`
          },
          {
            title: 'Making Budgets Stick in Practice',
            type: 'text',
            body: `A budget is only useful if department heads own it and review it regularly. CFA recommends monthly budget review meetings where each department head presents their actual-vs-budget performance, explains material variances (anything over 10%), and commits to corrective actions. These meetings should take no more than 90 minutes and follow a standard agenda.

              The biggest barrier to budget discipline in African hospitals is the belief that budgets are accounting exercises disconnected from clinical operations. To overcome this, CFA consultants help clients build budgets from the bottom up with department-level input, link budget performance to departmental KPIs, and make variance reports simple enough for clinical managers (not just accountants) to understand. When the head of surgery can see that theatre underutilization is costing the hospital 15M NGN per month in lost revenue, budget conversations become operational conversations.`
          }
        ],
        exercises: [
          {
  ...
  await prisma.trainingQuestion.createMany({
    data: [
      ...
      },
      {
        moduleId: m4_2.id,
        type: 'MULTIPLE_CHOICE',
        question: 'What contingency percentage does CFA recommend building into hospital operating budgets?',
        options: JSON.stringify([
        ...
        explanation: 'CFA recommends a 5-10% contingency for unexpected cost increases such as generator fuel during power outages or emergency drug procurement.',
        points: 1,
        order: 3,
      },
      {
        ...
        explanation: 'CFA recommends monthly budget review meetings where each department head presents actual-vs-budget performance, explains material variances (over 10%), and commits to corrective actions.',
        points: 1,
        order: 4,
      },
      {
  ...
  const professionalStandards = await prisma.trainingTrack.create({
    data: {
      ...
      description: `Every CFA consultant represents the firm in every client interaction. This track covers the
        ethical standards, codes of conduct, and data protection obligations that govern how we work.
        You will learn the CFA Code of Conduct, understand your obligations under the Nigeria Data Protection
        Regulation (NDPR) and healthcare data privacy standards, and develop the professional discipline that
        distinguishes CFA consultants in the market.`,
      level: 'FOUNDATION',
      category: 'methodology',
  ...
  // Module 5.1: CFA Code of Conduct & Ethics
  const m5_1 = await prisma.trainingModule.create({
    data: {
      trackId: professionalStandards.id,
      name: 'CFA Code of Conduct & Ethics',
      slug: 'cfa-code-of-conduct-ethics',
      description: 'Understand the ethical principles and professional conduct standards that every CFA consultant must uphold, including conflict of interest management, anti-corruption obligations, and client relationship boundaries.',
      order: 1,
      estimatedMinutes: 60,
      passingScore: 80,
      content: {
      ...
      resources: {
        links: [
          { title: 'CFA Code of Conduct (Full Document)', url: 'internal://knowledge/code-of-conduct' },
    ...
    data: [
      {
        moduleId: m5_1.id,
        type: 'MULTIPLE_CHOICE',
        question: 'What are CFA\'s four core ethical principles?',
        options: JSON.stringify([
        ...
        explanation: 'CFA operates on four non-negotiable ethical principles: integrity (tell the truth), objectivity (evidence-driven recommendations), confidentiality (protect client information), and competence (deliver quality work).',
        points: 1,
        order: 1,
      },
      {
        ...
        explanation: 'CFA has zero tolerance for facilitation payments. Any gray-area situation must be escalated immediately. There is no penalty for escalating, but there are severe consequences for not escalating.',
        points: 1,
        order: 2,
      },
      {
        moduleId: m5_1.id,
        type: 'MULTIPLE_CHOICE',
        question: 'Your analysis shows that a hospital does not need a full transformation program and a 6-week focused intervention would solve the problem. What should CFA recommend?',
        options: JSON.stringify([
          { id: 'a', text: 'The full transformation program because it generates more revenue for CFA', isCorrect: false },
          ...
          { id: 'c', text: 'A compromise: a medium-sized engagement that balances client needs and CFA revenue', isCorrect: false },
          { id: 'd', text: 'Let the client decide between the two options without making a recommendation', isCorrect: false },
        ]),
        explanation: 'Objectivity means recommendations are driven by evidence, not by what generates the largest engagement. If a 6-week intervention solves the problem, that is what CFA recommends. Long-term reputation depends on prioritizing client outcomes.',
        points: 1,
        order: 3,
      },
      {
        ...
        options: JSON.stringify([
          ...
          { id: 'c', text: 'Resign from CFA to avoid the conflict', isCorrect: false },
      ...
      },
      {
        moduleId: m5_1.id,
        type: 'MULTIPLE_CHOICE',
        question: 'What is the maximum value of a personal gift a CFA consultant may accept from a client?',
        options: JSON.stringify([
        ...
        explanation: 'CFA consultants may only accept token gifts valued under $25. Anything above that threshold must be declined to maintain professional independence and avoid the appearance of impropriety.',
        points: 1,
        order: 5,
      },
    ],
  ...
  const m5_2 = await prisma.trainingModule.create({
    data: {
      ...
      description: 'Understand your obligations under the Nigeria Data Protection Regulation (NDPR), healthcare data privacy standards, and CFA confidentiality policies that protect both client and patient information.',
      order: 2,
      estimatedMinutes: 60,
      passingScore: 80,
      content: {
        sections: [
          {
            ...
            type: 'text',
            body: `The Nigeria Data Protection Regulation (NDPR), issued in 2019 and strengthened by the Nigeria Data Protection Act of 2023, governs how personal data is collected, processed, stored, and shared. For CFA consultants, this is directly relevant because hospital engagements routinely involve access to patient records, staff personnel files, and financial data containing personal information. NDPR requires lawful basis for processing, data minimization (collect only what you need), purpose limitation (use data only for the stated purpose), and storage limitation (do not keep data longer than necessary).

              Health data receives special protection under NDPR as "sensitive personal data." Processing health data requires explicit consent from the data subject or a legal basis such as public health necessity. When CFA consultants access patient records during a diagnostic, they must work with anonymized or pseudonymized datasets wherever possible. If individual patient records are needed (e.g., for clinical pathway analysis), the client must provide the legal basis and CFA must follow strict handling protocols.`
          },
          {
            title: 'CFA Data Handling Protocols',
            type: 'text',
            body: `CFA maintains strict data handling protocols that go beyond legal minimums. Client data must never be stored on personal devices or in personal cloud accounts. All data stays within the CFA platform or approved secure storage. Laptops used for client work must have full-disk encryption enabled. Data shared between team members must use the CFA platform, not email attachments or messaging apps.

              When an engagement ends, all client data must be returned or securely deleted within 30 days unless a longer retention period is agreed in the engagement contract. CFA maintains a data destruction register that documents what was deleted, when, and by whom. Consultants who retain client data after an engagement ends are in violation of CFA policy and potentially in breach of NDPR. The engagement lead is responsible for ensuring the data destruction process is completed and documented.`
          },
          {
            ...
            body: `Confidentiality extends beyond data files to everyday behavior. Do not discuss client details in public spaces, taxis, hotel lobbies, or restaurants. Do not leave client documents visible on your screen in airports or co-working spaces. Do not mention client names or details on social media, including LinkedIn posts about "exciting work" that could identify the client.
              ...
              Within CFA, confidentiality follows the need-to-know principle. You may discuss client matters with team members assigned to the same engagement and with CFA leadership for quality review purposes. You may not share client information with CFA colleagues on other engagements, even if they are working with a similar type of institution. If a colleague asks about your engagement, redirect them to the engagement lead. Client trust is earned one interaction at a time and lost in a single careless moment.`
          }
        ],
        exercises: [
          {
            title: 'Practice: Data Handling Scenario',
            instruction: 'You are conducting a diagnostic at a hospital and need to analyze patient readmission patterns over the past 12 months. The hospital offers to give you a full extract of their patient database including names, national ID numbers, and clinical diagnoses. Describe step by step how you would handle this situation under NDPR and CFA data handling protocols.',
          },
        ]
      },
      resources: {
        links: [
          ...
          { title: 'CFA Data Handling Policy', url: 'internal://knowledge/data-handling-policy' },
        ],
  ...
  await prisma.trainingQuestion.createMany({
    data: [
      ...
      },
      {
        moduleId: m5_2.id,
        type: 'MULTIPLE_CHOICE',
        question: 'When a CFA engagement ends, within what timeframe must client data be returned or securely deleted?',
        options: JSON.stringify([
        ...
        explanation: 'CFA policy requires all client data to be returned or securely deleted within 30 days of engagement completion, unless a longer retention period is specified in the engagement contract.',
        points: 1,
        order: 2,
      },
      {
        moduleId: m5_2.id,
        type: 'MULTIPLE_CHOICE',
        question: 'You need to analyze patient records for a clinical pathway review. What is the preferred approach under CFA data handling protocols?',
        options: JSON.stringify([
          ...
          { id: 'd', text: 'Email the records to the CFA analytics team for processing', isCorrect: false },
        ]),
        explanation: 'CFA protocols require using anonymized or pseudonymized datasets wherever possible. If individual patient records are needed, the client must provide the legal basis and CFA must follow strict handling protocols.',
        points: 1,
        order: 3,
      },
      {
        moduleId: m5_2.id,
        type: 'MULTIPLE_CHOICE',
        question: 'A CFA colleague on a different engagement asks about your current project because they are "working on something similar." What should you do?',
        options: JSON.stringify([
          { id: 'a', text: 'Share details since they are a CFA employee and bound by the same confidentiality policies', isCorrect: false },
        ...
        explanation: 'Within CFA, confidentiality follows the need-to-know principle. Client information may only be shared with team members on the same engagement and CFA leadership for quality review. Colleagues on other engagements should be redirected to the engagement lead.',
        points: 1,
        order: 4,
      },
      {
        moduleId: m5_2.id,
        type: 'MULTIPLE_CHOICE',
        question: 'Which of the following is a violation of CFA data handling protocols?',
        options: JSON.stringify([
          { id: 'a', text: 'Storing client data on the CFA platform', isCorrect: false },
        ...
        explanation: 'Data shared between team members must use the CFA platform, not email attachments or messaging apps like WhatsApp. Client data must never be stored on personal devices or in personal cloud accounts.',
        points: 1,
        order: 5,
      },
    ],
  ...
  console.log('\nCFA Training Academy Foundation Level (B) seeding complete!')
