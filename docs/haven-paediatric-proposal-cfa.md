...
## 1. Why we are having this conversation
...
Before any audit, the fortnightly report already surfaces the levers. We lead with this because it shows leadership we see the business clearly.
...
I would structure this as a single board-led turnaround with five workstreams. The first four are the core project. The fifth is an optional, ongoing oversight retainer that leadership can opt into.
...
**Workstream 5. Board-level management oversight (optional retainer).** Up to two days per month of partner-level oversight from me or a delegate, continuing after the core project. This is the ongoing governance layer, offered as an opt-in.

---

## 5. The senior operations leader

One thing I advised early still holds, and leadership is now ready for it: Haven needs a senior, experienced operations leader running the facility day to day. The audit will define that role and its KPIs first, so the hire lands into clarity rather than chaos. Consult for Africa can source and vet that person through CadreHealth, our workforce platform, including diaspora and senior operator profiles. The right sequence is to define the role during the engagement, then recruit into it.
...
| Culture and process build | Months 2 to 4 | Standards of work, incentives, procurement and inventory, reporting |
...
The principle here is simple: we price every workstream at standard Consult for Africa rates so the board sees the real value, then show a single partner discount line, then the net. Nothing is hidden in the rate.

Given our longstanding relationship, and that I have been invited to Haven's board, the facility receives a 40 percent discount on every line.
