...
interface Deliverable {
  ...
  version: number;
  assignment: { consultant: { name: string } } | null;
}
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-3 mt-2 text-xs text-gray-400">
                  {d.assignment && <span>{d.assignment.consultant.name}</span>}
                  {d.submittedAt && (
                    <>
                      <span>·</span>
              ...
              {(d.status === "SUBMITTED" || d.status === "IN_REVIEW" || d.status === "NEEDS_REVISION") && (
                <Link
                  ...
                  className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-opacity hover:opacity-80"
