...
export function ReEngagementButton() {
  ...
  const [reengageCount, setReengageCount] = useState<number | null>(null);
  const [stage, setStage] = useState<Stage>("idle");
  const [result, setResult] = useState<SendResult | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/admin/cadrehealth/send-reengagement-emails")
      .then((r) => r.json())
      .then((data) => setReengageCount(data.reengageCount ?? 0))
      .catch(() => setReengageCount(null));
  }, []);
  ...
  async function send() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/cadrehealth/send-reengagement-emails", {
        method: "POST",
      });
  ...
  if (reengageCount === 0) return null;
  ...
  if (stage === "idle") {
    return (
      <button
        ...
        title="Send a follow-up email to professionals who got the original outreach but never tried to claim"
      >
        <Send className="h-3.5 w-3.5" />
        Re-engage emailed users
        {reengageCount !== null && reengageCount > 0 && (
  ...
  if (stage === "confirming" && reengageCount !== null) {
    return (
      <div
      ...
      >
        ...
        <span className="font-medium" style={{ color: "#0F2744" }}>
          Send re-engagement email to {reengageCount} user{reengageCount === 1 ? "" : "s"}?
        </span>
        <button
          type="button"
  ...
  if (stage === "sending") {
    return (
      <span className="inline-flex items-center gap-2 rounded-xl bg-white/95 px-4 py-2.5 text-sm font-semibold text-gray-700">
        ...
        Sending {reengageCount} re-engagement email{reengageCount === 1 ? "" : "s"}...
      </span>
    );
  }
