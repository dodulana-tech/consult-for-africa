...
import {
  DENNIS_ASHLEY_SURVEYS,
  tensionSide,
  type SurveyMeta,
  type ScaleQuestion,
} from "@/lib/dennis-ashley-survey";
import {
  DA_ENGAGEMENT,
  PRIORITY_NINE,
  REQUEST_SECTIONS,
  sectionLabel,
} from "@/lib/dennis-ashley-audit";
    ...
    <section style={{ ...CARD, padding: 22 }} className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm" style={{ color: "#6B7280" }}>
            Uploaded from{" "}
            <a href="/DennisAshleyProject" className="underline" style={{ color: "#1F7A8C" }}>/DennisAshleyProject</a>.
                  ...
                  {files.map((f) => (
                    <tr key={f.id} style={{ borderTop: "1px solid #f1f5f9" }}>
                      <td className="py-1.5 pr-3">
                        <a href={`/api/dennis-ashley-audit/upload/${f.id}`} className="underline" style={{ color: "#1F7A8C" }}>
                          {f.filename}
                        </a>
                        {f.note && (
...
export default async function DennisAshleyAuditPage() {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <TopBar
        title="Dennis Ashley Audit Documents & Surveys"
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {DENNIS_ASHLEY_SURVEYS.map((meta) => {
