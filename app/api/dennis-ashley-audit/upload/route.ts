...
// Public, unauthenticated: the Dennis Ashley team answering the information
// request straight from /DennisAshleyProject, because asking a clinic to set up
...
function auditKey(section: string, filename: string): string {
  ...
  return `${FOLDER}/dennis-ashley-audit/${section}/${rand}${ext ? `.${ext}` : ""}`;
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[dennis-ashley-audit/upload] presign failed", err);
...
export async function PUT(req: NextRequest) {
  ...
  if (!d.storageKey.startsWith(`${FOLDER}/dennis-ashley-audit/`)) {
  ...
  try {
    ...
    await notifyInternal(
      ...
      `Dennis Ashley: ${d.filename} uploaded (section ${d.section}, ${total} in)`,
      `<div style="font-family:Helvetica,Arial,sans-serif;font-size:15px;line-height:1.6;color:#1F2937">
<p><b>${d.uploadedBy?.trim() || "Someone at Dennis Ashley"}</b> uploaded <b>${d.filename}</b> against section <b>${d.section}</b>.</p>
${d.note?.trim() ? `<p style="color:#475569">Their note: ${d.note.trim()}</p>` : ""}
<p>That is <b>${total}</b> document${total === 1 ? "" : "s"} received so far.</p>
<p><a href="https://www.consultforafrica.com/admin/dennis-ashley-audit" style="color:#0B3C5D">See what has come in</a></p>
</div>`
    );
  } catch (err) {
    console.error("[dennis-ashley-audit/upload] notification failed", err);
  }
