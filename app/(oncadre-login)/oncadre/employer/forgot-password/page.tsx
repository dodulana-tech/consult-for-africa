...
export default function EmployerForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/cadre/employer/forgot-password", {
        method: "POST",
    ...
    <div className="flex min-h-screen items-center justify-center p-6" style={{ background: "#FAFBFC" }}>
      <div className="w-full max-w-md">
        <div className="mb-8">
          ...
          <p className="mt-1 text-xs font-medium uppercase tracking-[0.2em]" style={{ color: "#D4AF37" }}>
            Employer Portal
          </p>
        </div>
        ...
        {sent ? (
          ...
          <div className="rounded-2xl border bg-white p-8 shadow-sm" style={{ borderColor: "#E8EBF0" }}>
            ...
            <p className="mt-6 text-center text-sm text-gray-500">
              ...
              <Link href="/oncadre/employer/login" className="font-semibold text-[#0B3C5D] hover:underline">
                Sign in
              </Link>
            </p>
          </div>
