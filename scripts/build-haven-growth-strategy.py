...
DOCS = ROOT / "docs"
SRC = DOCS / "haven-growth-strategy-cfa.md"
OUT = DOCS / "haven-growth-strategy-cfa.pdf"
...
def content_bg(c, doc):
    ...
    c.drawRightString(PAGE_W - MARGIN, PAGE_H - 23, "Haven Paediatric Centre  /  Growth Strategy")
    c.setFillColor(GOLD)
...
def build():
    ...
    doc = BaseDocTemplate(
        ...
        title="Haven Paediatric Centre - Growth Strategy",
    ...
    el.append(Paragraph("Growth strategy: ten levers to N45-50M a month",
                        ParagraphStyle("cs", fontName="Helvetica-Bold", fontSize=15,
                                       leading=20, textColor=GOLD)))
    el.append(Spacer(1, 26))
    el.append(Paragraph(
        "The answer first, then the detail: ten essential growth levers, grouped into five jobs and "
        "led by the NICU, that take Haven from N15-20M to N45-50M a month at an 18-20% EBITDA floor "
        "in six months.",
