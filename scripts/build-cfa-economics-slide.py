"""Build standalone economics / transparency slide.

Shows the full breakdown of every naira a hospital pays:
- VAT and WHT (statutory, neutral)
- CFA platform fee
- Employer-side statutory costs (pension, NSITF, ITF) where applicable
- Professional gross
- PAYE / pension / NHIS deductions
- Professional take-home

Three worked examples covering the full motion:
1. Senior fractional (contractor model)
2. Mid-level secondment (employer-of-record model)
3. Locum shift (per-shift contractor)
"""
...
def waterfall_row(slide, x, y, w, label, amount, kind="neutral"):
    """One row in a breakdown card. kind: gross/cfa/pro/net/deduction/neutral."""
    color_map = {
        "gross": NAVY,
        "cfa": GOLD,
        "pro": TEAL,
...
set_text(tf, "Senior contractor model. Professional invoices CFA, handles own taxes.",
         size=8, color=GREY)
...
ry += Inches(0.5)
tf = add_textbox(s, x + Inches(0.15), ry, card_w - Inches(0.3), Inches(0.4))
set_text(tf, "CFA is employer of record. Full statutory compliance handled.",
         size=8, color=GREY)
...
tf = add_textbox(s, x + Inches(0.15), cards_y + Inches(0.27), card_w - Inches(0.3), Inches(0.3))
set_text(tf, "Medical Officer shift at N100K", size=12, bold=True, color=NAVY)

ry = cards_y + Inches(0.7)
waterfall_row(s, x + Inches(0.15), ry, card_w - Inches(0.3), "Hospital pays per shift", "N100,000", "gross")
ry += Inches(0.38)
light_row(s, x + Inches(0.15), ry, card_w - Inches(0.3), "- WHT 5% (individual, FIRS)", "N5,000")
ry += Inches(0.4)
waterfall_row(s, x + Inches(0.15), ry, card_w - Inches(0.3), "CFA platform margin (12%)", "N12,000", "cfa")
ry += Inches(0.42)
waterfall_row(s, x + Inches(0.15), ry, card_w - Inches(0.3), "Professional payout (instant)", "N83,000", "pro")
ry += Inches(0.38)
...
ry += Inches(0.4)
waterfall_row(s, x + Inches(0.15), ry, card_w - Inches(0.3), "Net cash to professional", "N83,000", "net")
# Footnote
ry += Inches(0.5)
tf = add_textbox(s, x + Inches(0.15), ry, card_w - Inches(0.3), Inches(0.4))
set_text(tf, "Per-shift contractor. Paid via Paystack within 24 hours of shift.",
         size=8, color=GREY)

# Bottom strip: principles
strip_y = Inches(7.0)
...
output_path = "/Users/debo/consult-for-africa/docs/cfa-economics-transparency-slide.pptx"
